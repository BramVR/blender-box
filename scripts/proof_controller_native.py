...
SOURCES = ("scripts/proof_controller.py", "scripts/proof_controller_native.py", "scripts/proof_controller_store.py",
           ...
           "scripts/proof_qualification_fixture.py",
...
class QualificationAuthorization:
    ...
    def validate(self, policy, installation, case, now, *, launch):
        value = self.value
        model.require(policy.value["variant"] in ("baseline", "named-target")
                      and case in value["cases"] and value["policy_sha256"] == policy.digest
...
class Selector:
    ...
    def parse(cls, raw):
        ...
        if isinstance(value, dict) and set(value) == {"schema_version", "execution_id", "attempt", "request_digest", "mode"}:
            return cls.create("operational", parse_intent(raw))
...
def qualification_dispatch(controller, operation, context):
    if operation != "stop":
        if operation == "recover" and context.case == "windows-reboot-recover" and context.cleanup is None:
...
def windows_qualification_command(command, policy, files, ops):
    ...
    model.require(authority.digest == command["authorization_sha256"]
                  and authority.value["authorization_id"] == command["authorization_id"],
                  "qualification-authorization-changed")
    context = WindowsQualification(authority, command["case"])
    operation = command["operation"]
    root = CONTROL / context.execution_id
    validate_windows_qualification(context, policy, files,
                                   launch=operation == "start" and not files.exists(root), observe=operation == "status")
    ...
    if operation == "start":
        if not files.exists(root):
            validate_windows_inputs(policy)
        ...
        controller.dispatch(model.Command("start", context.execution_id, request))
        if context.case in ("windows-crash-recover", "windows-reboot-recover"):
            with controller.locked():
                state = controller.load(root)
                active = files.exists(root / "active-checkpoint.json")
                pending = state["attempt"] == 1 and (
                    (state["phase"] == "running" and not active)
                    or (context.case == "windows-crash-recover" and active and not files.exists(root / "interruption.json")
                        and state["phase"] in ("running", "unresolved")))
            if pending:
                if not active:
...
def linux_observe(intent, files, ops, *, stop=False):
    ...
    if intent.value["case"] == "initiator-disconnect" and boot == receipt.owner.boot_id and not ops.whole_empty():
        if files.exists(root / "initiator.json"):
            ...
            try:
                ...
                gone = True
            channel = root / "initiator-channel-lost.json"
            if files.exists(channel):
                model.require(model.document(files.read(channel, 4096)) ==
                              {"schema_version": 1, "intent_sha256": intent.digest}, "qualification-initiator-invalid")
            if gone and files.exists(channel) and not files.exists(root / "initiator-disconnected.json"):
                owner = receipt.owner
    ...
    if case == "initiator-disconnect" and files.exists(root / "initiator-disconnected.json"):
        model.require(model.document(files.read(root / "initiator-channel-lost.json", 4096)) ==
                      {"schema_version": 1, "intent_sha256": intent.digest}, "qualification-evidence-changed")
...
def accept_active_checkpoint(controller, context, files, ops):
    ...
    with controller.locked():
        ...
        accepted = files.exists(active_path)
        model.require(state["attempt"] == 1 and (state["phase"] == "running" or
                      (accepted and context.case == "windows-crash-recover" and state["phase"] == "unresolved"
                       and state["closed"] and state["proof_result"] == "fail")), "qualification-run-not-active")
        job = controller.job(root, state)
        ...
        model.require(bound is not None and (accepted or bound.process == "live") and bound.release == "uncertain"
        ...
        if accepted:
            model.require(model.document(files.read(active_path, 64 << 10)) == record
                          and state["recovery_inputs"] == retained, "qualification-checkpoint-changed")
        else:
            model.require(not files.exists(root / "interruption.json"), "qualification-checkpoint-changed")
            state["recovery_inputs"] = retained
            controller.save(root, state)
            files.publish(active_path, model.proof.canonical(record))
        if context.case == "windows-crash-recover":
            ...
            if state["local_termination"] != "proven":
                ...
                model.require(bound.process == "gone", "qualification-native-changed")
            interruption = root / "interruption.json"
            if not files.exists(interruption):
                files.publish(interruption, model.proof.canonical({"schema_version": 1,
                              "active_checkpoint_sha256": model.proof.digest(files.read(active_path, 64 << 10)),
...
def load_prepared(*, windows=False, launch=False):
    ...
    qualification_raw = protected_read(CONFIG / "qualification.json", model.MAX_FILE, private=True)
    installing = policy.value["variant"] == "host-install"
    achieved = model.document(qualification_raw, version=2 if installing else 1)
    fields = {"schema_version", "qualified", "policy_sha256", "evidence"} | ({"scope"} if installing else set())
    model.require(set(achieved) == fields and (not installing or achieved["scope"] == INSTALL_PREFLIGHT_SCOPE),
                  "native-adapter-unqualified")
    model.require(type(achieved["qualified"]) is bool and achieved["policy_sha256"] == policy.digest
                  and isinstance(achieved["evidence"], dict)
                  and set(achieved["evidence"]) == set(qualifications(policy.value["variant"])),
                  "native-adapter-unqualified")
    if achieved["qualified"]:
        policy.qualify(qualification_raw)
...
def validate_windows_inputs(policy):
...
def wait_channel_loss(fd, deadline_unix):
    ...
    model.require(not any(mask & select.POLLNVAL for _, mask in events), "qualification-channel-invalid")
    return any(mask & (select.POLLERR | select.POLLHUP) for _, mask in events)
