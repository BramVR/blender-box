    ...
    def serve(self):
        ...
        with self.ops.group(native.UNIT_CGROUP + "/" + leaf) as group:
            ...
            try:
                ...
                completed = (self.collect(receipt, result_parent) if request is not None and request.variant == "host-install"
                             else receive(result_parent, 64 << 10 if type(self.context) is native.LinuxIntent else model.MAX_FILE))
