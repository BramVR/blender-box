...
def baseline(request, commands_factory=Commands, host=None, *, native_authority=None):
    ...
    report = {"schema_version": 1, "proof": host.name + "-" + ("named-target" if named else "baseline"),
    ...
    try:
        ...
        if observed["host_sha256"] != host_hash:
            ...
            setup_budget = {"timeout": 420, "cleanup_grace": 65} if host.platform == "windows" else {"timeout": 300}
            applied = commands.json(setup_args + ["--apply"], **setup_budget)
