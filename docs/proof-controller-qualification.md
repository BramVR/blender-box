...
Capacity refusal preserves existing records and cleanup authority. Expired launch authority cannot start new work; an exact retry of an existing execution uses the recovery deadline. Root status remains available after expiry or promotion.
...
If checkpoint collection is interrupted, retry the same `start` authorization to accept the retained checkpoint. A renewed cleanup authorization can recover the original Windows Run after a canceled reboot; it does not require reboot qualification evidence and leaves the case partial or failed.
