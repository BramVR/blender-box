...
Preview includes the temporary launcher policy in its plan digest. Apply creates one limited interactive task with no triggers, `IgnoreNew`, a unique execution name, and a six-minute Scheduler backstop. Its keeper enforces the immutable five-minute deadline even if SSH disconnects. The task remains until the waiting caller or exact recovery proves keeper exit and deletes its inactive definition. An unknown outcome blocks new Runs and unrelated setup operations. Missing process IDs or an absent task do not establish cleanup. If the keeper dies before recording proof, or launcher registration, Run, deletion, or an installed-task mutation remains unsettled, preserve the state for operator review.

An installer command completes only after its root and all owned children exit naturally within the operation deadline. Children may finish after their root; cancellation or an expired deadline still fails the command and starts exact cleanup. Process cleanup requires exact retained process handles and complete Job membership evidence within one five-second cleanup deadline. A missed fast child, failed process query, or exceeded collection limit leaves cleanup unknown even when the root process has exited. Preserve the pending fence and receipts in that state; do not clear them because the task list looks empty. The [ownership decision](architecture/0007-windows-installation-ownership.md) records the bounded installer workload assumptions and required native proof.
