...
After a process-creation attempt, `not-started` requires a failed return with no process or thread identity and a successful fresh Job accounting query reporting zero total and active processes. Partial identity, any observed process history, or an accounting fault leaves cleanup unknown after bounded Job settlement. Later empty accounting cannot erase that uncertainty. An impossible successful return without a process handle also remains unknown. These paths publish neither `not-started` nor process-tree exit proof.

Native setup probes own a separate, unnamed Windows Job with kill-on-close. Windows 10 or later assigns each probe to that Job atomically at process creation through `PROC_THREAD_ATTRIBUTE_JOB_LIST`. Only its standard input and output handles are inherited. Owned command roots start suspended; exact root identity, Job-member collection, and any admission gate are ready before resume. Generic commands, Python probes, and admitted workers use `DETACHED_PROCESS`. The trusted PowerShell adapter selects a separate launch intent that omits `DETACHED_PROCESS` and retains `CREATE_NO_WINDOW`. A native comparison showed detached PowerShell exiting before script entry with no output, while removing detachment from generic commands left console helpers active at root exit. Any console helper remains subject to the same exact Job cleanup proof. This authority covers only the probe tree created by setup and never an existing Session. Atomic assignment avoids the orphan interval between creating a process and assigning it to a Job, as described by [Microsoft](https://devblogs.microsoft.com/oldnewthing/20230209-00/?p=107812).
