...
The user supplies one non-secret target file or saved target name and one explicit Run Payload. The public Run commands cover inspection, execution, recovery, and exact stop:
...
blender-box plan --target target.json --payload payload.json --json
blender-box doctor --target target.json --payload payload.json --json
...
Each command also accepts `--target-name NAME` in place of `--target PATH`. Named import and storage are defined by the [target contract](target-contract.md).

`run` returns a Run ID before remote work starts. Its terminal JSON names the request identity, exact daemon Session identity, Evidence Bundle, and cleanup state. The client records the original target fingerprint and complete claim before inspection. If the connection drops, `status` first matches the selected configuration against that local authority, then validates the host-owned receipt. `stop` settles only the exact original claim and accepted Session identity.
...
type Runner interface {
    Plan(PlanIntent) (PlanResult, error)
    Doctor(context.Context, PlanIntent) (DoctorResult, error)
...
type HostAdapter interface {
    Inspect(context.Context, Target, HostRequirements) (HostInspection, error)
...
The exported methods reflect user operations. `Plan` validates declared files and capture types without contacting the host. `Doctor` combines that plan with a read-only host capability report. `Runner.Run` owns the complete state machine and always attempts settlement after a lease exists. Callers never coordinate adapter phases themselves. `HostAdapter` is a system boundary for the remote Windows implementation and fakes, not a public user API.
...
Read-only inspection proves that the SSH controller, console user, and interactive task share one SID. It also proves explicit controller FullControl on the sealed state roots, executable directories, and executable files; runtime state descendants may inherit that authority only from the sealed roots. `Setup` validates the target contract before SSH, creates and seals both OS-released lock files, and inspection requires `.operation.lock` and `.launch.lock` to remain regular non-reparse files with the declared authority. The launch fence stays held through exact Session identity publication in the Host Lock and receipt. A daemon start error with a returned or recoverable identity records that identity before failure so settlement can stop it exactly. If the exact rollback stop succeeded but its response was lost, settlement requires the launch fence to be free, republishes only the receipt's exact identity, and repeats the idempotent stop before cleanup. Only the shared identity and privileged Windows principals may own or hold replacement authority on ancestors of the work root. The work root uses a narrow legacy-SCP-safe ASCII path grammar, enforced by target validation and again by the upload transport; target validation reserves the exact 44-character setup staging suffix before accepting a root and the longest 46-character atomic-replacement suffix after the host executable. Setup uploads a private immutable snapshot of the bytes it bounded and hashed. A complete Run accepts exactly one Scenario Result and exactly one file for each requested viewport, Blender-window, or desktop capture, with no duplicate or unsolicited evidence types. Schema 2 records fixed source paths, media types, dimensions, capture methods, hashes, sizes, and exact Session identities. The decoded Scenario Result remains capped at 1 MiB; daemon process output allows three times that size plus fixed envelope headroom for Python's worst-case nested JSON escaping. Cleanup walks the Run tree bottom-up, rejecting reparse points and non-regular descendants before deletion. The public `stop` command performs its final durable receipt observation under a fresh bounded reconciliation context, so caller cancellation during settlement cannot suppress the returned exact Session identity and known cleanup state. Repeating exact stop for a settled Run returns its known cleanup even while a different valid Run owns the Host Lock, without changing the newer Run's lock or pending request.
...
- Automated fakes prove the Blender-window and desktop contracts. Exact owned-host visual proof remains opt-in.
...
The public `windows setup`, `plan`, `doctor`, `run`, `status`, and `stop` commands now enter through the boundaries above. Setup is plan-only by default and uses bounded SCP staging plus independent guard, script, and binary hashes. One transfer aggregate owns the attempt's exact prepare guard, host binary, and setup-owner script paths. Cleanup starts with the first upload attempt, uses a fresh bounded context, removes only those three leaf paths, and treats missing files as success. It preserves the transfer when setup-owner cleanup is unverified. The Windows adapter carries versioned JSON over SSH; the static task invokes the same binary's private `host run-request` entry point. The host rehashes published payload bytes before launch, stores the exact daemon Session identity in the Host Lock, waits for exact-session health, and fences every daemon call and stop with it. Payload schema 2 adds distinct Blender-window and opt-in desktop capture declarations. Every schema 2 evidence entry carries its source path, media type, byte size, hash, dimensions and method when applicable, and exact Session identity. Capture PNG dimensions are decoded before the host publishes its receipt and again after the client fetches the bytes. Cleanup keeps ownership proof until the last deletion and absorbs transient Windows log-handle release within its bounded settlement attempt.

`run` publishes its Run ID on stderr before validation or remote work while reserving stdout for one versioned success or failure JSON document. Local preflight failures do not contact the host. After a later orchestration failure and bounded settlement, JSON mode attempts one bounded `status` recovery so the result includes the durable receipt and cleanup facts; it preserves failure state when host completion preceded a local evidence failure. If the host is unreachable it reports only facts known locally. The returned Evidence Bundle contains a no-replace `manifest.json`, terminal `evidence.json`, the Scenario Result, and exactly the requested capture files with typed provenance. `status` provides a reconnect view; `stop` recovers the exact claim and Session authority from host state, including interrupted receipt publication, partial acquisition, and final-receipt crash windows.
