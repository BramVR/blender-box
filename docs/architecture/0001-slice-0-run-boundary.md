...
The Windows task runs as the logged-in controller identity with `LogonType=Interactive`, limited rights, no trigger, `IgnoreNew`, and no execution time limit. The SSH controller and interactive task must resolve to the same SID, as defined in ADR 0002. [Owned installation](0006-windows-installation-ownership.md) now provisions the exact runtime and marked task through a host-local operation. It validates identity, trusted physical paths, and generated target configuration before mutation; maintenance shares the Run operation/launch fences. Legacy SSH setup apply refuses unowned replacement. The task still starts the host entry point, which validates the request and lease before invoking `blendersessiond`. Blender's MCP port remains Windows-loopback-only.
