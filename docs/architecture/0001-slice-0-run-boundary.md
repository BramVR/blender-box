...
The Linux adapter now uses the same seven-method boundary and durable Run state machine. Linux request and settlement schema 2 bind the reviewed runtime without changing Windows schema 1. The [Linux host boundary](0004-linux-host.md) records its static user service, fresh-start preparation, provenance, and acceptance gaps.
