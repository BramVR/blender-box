...
Preparation creates one dedicated Ed25519 key and retains the offer and immutable enrollment intent under the private configuration root. Standard `ssh-keygen` generates the private-key format and derives its public identity. The client exports only the public request. A repeated preparation with the same name and offer returns the original intent after checking its key. A different intent or preexisting target name refuses.
