...
The runner persists the validated Run ID in its public failure result as soon as the CLI emits it. If a command then fails, the runner attempts bounded recovery through public `status` and `stop`. Successful recovery adds the verified request ID, request hash, and Session ID to the public result. The Session ID is `null` when the Run failed before a Session existed. Keep the private journal and receipts when cleanup remains unknown. The public result retains the verified recovery identity after a hosted controller is gone. Do not reset the fixture, remove host state, or stop Blender by name to make the proof pass.
