...
Direct hosted execution still fails preflight with `hosted-recovery-retention-unavailable`. Hosted installation now uses the [persistent controller](proof-controller.md), which retains private installer and original Run authority across loss of the GitHub runner. Native dispatch refuses unless qualification matches the exact artifacts, policy, and installer inputs. Environment approval alone cannot qualify it. No native or hosted qualification is claimed by the local tests.
...
A manually dispatched job is explicit premerge evidence, not a PR ruleset status check. Inspect the exact host-install run and receipt; missing, skipped, stale, or failed runs do not pass. See GitHub’s [required-check limitations](https://docs.github.com/en/enterprise-cloud@latest/pull-requests/how-tos/merge-and-close-pull-requests/troubleshooting-required-status-checks).

Host-install uses the separate protected `windows-onboarding-installer` environment. Its job verifies immutable driver and candidate checkouts, then uses the trusted driver's `proof_dispatch.py` to contact the qualified persistent controller. The controller executes its separately enrolled candidate checkout. The ephemeral runner receives only a dedicated forced-command dispatch key, controller trust, and scoped Tailnet access. Windows SSH keys, runtime inputs, and recovery records remain private on the persistent controller.
...
Before enrollment, review the exact environment settings, authorized candidate policy, SSH account/key scope, Tailnet tag and SSH-only network permission, and fixture mutation inventory. Baseline and named-target still use fresh hosted runners with their current preflight guard. Host-install uses its qualified persistent Linux controller and a separately authorized absent Windows fixture. Do not enroll a shared Windows desktop as an unrestricted self-hosted runner.
