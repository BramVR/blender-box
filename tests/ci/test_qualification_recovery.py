...
class QualificationRenewalTests(unittest.TestCase):
    ...
    def original_run(self, deadline="2026-09-06T17:25:00Z"):
        ...
        self.fence = {"schema_version": 1, "run_id": "bbx_" + "a" * 32, "request_id": "req_" + "b" * 32,
                      "request_hash": "c" * 64, "deadline": deadline, "session_id": "bss_" + "d" * 32}
    ...
    def delayed_recovery_checkpoint(self, case="windows-crash-recover"):
        expires = datetime.now(timezone.utc) + timedelta(hours=1)
        value = self.authority.value | {"authorization_id": "wq-" + "9" * 32,
                  "cases": [case], "launch_deadline_unix": int(expires.timestamp()),
                  "recovery_deadline_unix": int(expires.timestamp()) + 3600,
                  "fixture_sha256": native.identity_digest("WindowsFixture", {name: self.policy.value["artifacts"][
                      str(native.BASE / "tests/fixtures/qualification-windows-hold" / name)]
                      for name in ("payload.json", "scenario.py")})}
        provisional = native.QualificationAuthorization.parse(model.proof.canonical(value))
        self.execution_id = provisional.execution_id(case)
        self.root = self.base.control / self.execution_id
        self.request = fixtures.baseline.request(self.execution_id, expires_at=datetime.fromtimestamp(
            value["launch_deadline_unix"], timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"))
        job = model.Job(self.request, self.base.jobs / self.execution_id, self.base.checkout,
                        self.base.policy.expected_client_sha256, 1)
        operator_raw = self.files.read(self.base.operator)
        operator = model.document(operator_raw)
        ssh_raw = self.files.read(Path(operator["ssh_config"]))
        connection = model.ssh_connection(ssh_raw, operator["target"]["ssh_alias"])
        contents = {"original-operator.json": operator_raw, "original-ssh-config": ssh_raw,
                    "operator.json": model.proof.canonical(operator | {"ssh_config": str(job.root / "inputs/ssh-config")}),
                    "target.json": model.proof.canonical(operator["target"]),
                    "ssh-config": model.normalized_ssh(connection, job.root / "inputs"),
                    "key": self.files.read(Path(connection["identityfile"])),
                    "known_hosts": self.files.read(Path(connection["userknownhostsfile"]))}
        manifest = {"schema_version": 1, "files": {name: model.proof.digest(raw) for name, raw in contents.items()},
                    "candidate_checkout": str(self.base.checkout), "config": str(job.config),
                    "expected_client_sha256": job.expected_client_sha256}
        value.update(operator_manifest_sha256=model.proof.digest(model.proof.canonical(manifest)),
                     target_sha256=model.proof.digest(contents["target.json"]))
        self.authority = native.QualificationAuthorization.parse(model.proof.canonical(value))
        self.context = native.WindowsQualification(self.authority, case)
        self.service = native.NativeService(self.policy, self.files, self.ops, self.context)
        self.controller = model.Controller(self.base.control, self.base.jobs, self.base.policy, self.service,
            files=self.files, admission=self.policy.admit, qualification=self.context)
        self.controller.dispatch(model.Command("start", self.execution_id, self.request))
        self.assertEqual(len(self.starts), 1)
        deadline = (datetime.now(timezone.utc) + timedelta(hours=2)).strftime("%Y-%m-%dT%H:%M:%SZ")
        marker, pin = self.original_run(deadline)
        public = self.fence | {"state": "calling"}
        checkpoint = {"schema_version": 1, "kind": "windows-active-checkpoint", "execution_id": self.execution_id,
                      "attempt": 1, "inputs_digest": self.job.inputs_digest, **self.fence,
                      "marker_sha256": model.proof.digest(self.files.read(self.job.output / "private/run-journal.json")),
                      "claim_sha256": model.proof.digest(marker), "session_pin_sha256": model.proof.digest(pin),
                      "client_sha256": self.job.expected_client_sha256, "target_sha256": value["target_sha256"],
                      "status_sha256": model.proof.digest(model.proof.canonical(public)),
                      "observed_at": datetime.now(timezone.utc).isoformat()}
        (self.job.attempt_root / "active-commands").mkdir(mode=0o700)
        self.files.publish(self.job.attempt_root / "active-commands/command-001.stdout", model.proof.canonical(public))
        self.files.publish(self.job.attempt_root / "active-checkpoint.json", model.proof.canonical(checkpoint))
        path = native.CONFIG / "qualification-authorizations" / (value["authorization_id"] + ".json")
        path.parent.mkdir(mode=0o700)
        self.files.publish(path, self.authority.raw)
        command = {"operation": "start", "authorization_id": value["authorization_id"],
                   "authorization_sha256": self.authority.digest, "case": self.context.case}
        return command

    def test_retried_start_accepts_delayed_active_checkpoint_without_relaunch(self):
        command = self.delayed_recovery_checkpoint()
        result = native.windows_qualification_command(command, self.policy, self.files, self.ops)
        self.assertEqual((result["phase"], result["attempt"]), ("unresolved", 1))
        self.assertTrue(self.files.exists(self.root / "active-checkpoint.json"))
        self.assertTrue(self.files.exists(self.root / "interruption.json"))
        self.assertEqual(len(self.starts), 1)
        self.assertEqual(self.ops.stop.call_count, 1)

    def test_retried_start_uses_recovery_deadline_after_launch_deadline(self):
        command = self.delayed_recovery_checkpoint()
        after_launch = self.authority.value["launch_deadline_unix"] + 1
        with mock.patch.object(native.time, "time", return_value=after_launch):
            result = native.windows_qualification_command(command, self.policy, self.files, self.ops)
        self.assertEqual((result["phase"], result["attempt"]), ("unresolved", 1))
        self.assertTrue(self.files.exists(self.root / "interruption.json"))
        self.assertEqual((len(self.starts), self.ops.stop.call_count), (1, 1))

    def test_command_authorization_id_must_match_named_document(self):
        command = self.delayed_recovery_checkpoint()
        wrong_id = "wq-" + "8" * 32
        path = native.CONFIG / "qualification-authorizations" / (wrong_id + ".json")
        self.files.publish(path, self.authority.raw)
        with self.assertRaisesRegex(model.ControllerError, "qualification-authorization-changed"):
            native.windows_qualification_command(command | {"authorization_id": wrong_id},
                                                 self.policy, self.files, self.ops)
        self.assertFalse(self.files.exists(self.root / "active-checkpoint.json"))
        self.assertEqual((len(self.starts), self.ops.stop.call_count), (1, 0))

    def test_retried_start_completes_interrupted_root_record_after_exact_stop(self):
        command = self.delayed_recovery_checkpoint()
        publish = self.files.publish
        def interrupted(path, raw, **kwargs):
            if path == self.root / "interruption.json":
                raise model.ControllerError("simulated-root-exit")
            return publish(path, raw, **kwargs)
        with mock.patch.object(self.files, "publish", side_effect=interrupted):
            with self.assertRaisesRegex(model.ControllerError, "simulated-root-exit"):
                native.windows_qualification_command(command, self.policy, self.files, self.ops)
        self.assertTrue(self.files.exists(self.root / "active-checkpoint.json"))
        self.assertFalse(self.files.exists(self.root / "interruption.json"))
        self.assertEqual(self.controller.load(self.root)["local_termination"], "proven")
        result = native.windows_qualification_command(command, self.policy, self.files, self.ops)
        self.assertEqual((result["phase"], result["attempt"]), ("unresolved", 1))
        self.assertTrue(self.files.exists(self.root / "interruption.json"))
        self.assertEqual(len(self.starts), 1)
    ...
    def test_reboot_cleanup_recovers_original_run_when_reboot_was_canceled(self):
        self.delayed_recovery_checkpoint("windows-reboot-recover")
        cleanup = self.renewal("7")
        result = native.cleanup_qualification_command(self.command(cleanup), self.policy, self.files, self.ops)
        self.assertEqual((result["phase"], result["attempt"]), ("recovering", 2))
        self.assertFalse(self.files.exists(self.root / "reboot-observation.json"))
        self.assertEqual([item["mode"] for item in self.starts], ["baseline", "recover"])
        self.assertEqual(self.ops.stop.call_count, 1)
