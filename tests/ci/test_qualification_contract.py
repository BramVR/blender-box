...
class LinuxQualificationLifecycleTests(unittest.TestCase):
    ...
    def test_finished_initiator_without_channel_loss_cannot_prove_disconnect(self):
        from contextlib import contextmanager
        self.intent = native.LinuxIntent.parse(linux() | {"case": "initiator-disconnect"})
        ...
        self.files.publish(self.intent.root / "initiator.json", model.proof.canonical(
            {"schema_version": 1, "boot_id": self.owner.boot_id, "pid": 99, "start": 100}))
        self.ops.whole_empty.return_value = False
        self.ops.unit.return_value = native.UnitState("active", self.owner.parent_pid,
                                                      self.owner.invocation_id, native.UNIT_CGROUP)
        def process(pid):
            if pid == 99:
        ...
        observed = native.linux_observe(self.intent, self.files, self.ops)
        self.assertEqual(observed["phase"], "released")
        self.assertFalse(self.files.exists(self.intent.root / "initiator-disconnected.json"))
        self.files.publish(self.intent.root / "initiator-channel-lost.json", model.proof.canonical(
            {"schema_version": 1, "intent_sha256": self.intent.digest}))
        native.linux_observe(self.intent, self.files, self.ops)
    ...
    def test_initiator_channel_wait_requires_reader_loss(self):
        ...
        try:
            self.assertFalse(native.wait_channel_loss(writer, time.time() - 1))
            observed = []
            thread = threading.Thread(target=lambda: observed.append(native.wait_channel_loss(writer, time.time() + 5)))
            thread.start()
            os.close(reader)
            reader = -1
    ...
    @unittest.skipUnless(hasattr(select, "poll"), "Linux channel polling")
    def test_invalid_output_descriptor_cannot_count_as_disconnect(self):
        reader, writer = os.pipe()
        os.close(reader)
        os.close(writer)
